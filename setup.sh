...
main() {
    ...
    echo "  5. (Optional) Copy .gitconfig.local.template to ~/.gitconfig.local for machine-specific git config"
    echo ""
    ...
    echo "  • Claude Code settings (.claude/ directory with custom status line)"
    echo ""
    print_status "Configuration notes:"
    echo "  • Claude Code: Custom status bar with git integration (~/.claude/settings.json)"
    echo "  • Git config: Override defaults by creating ~/.gitconfig.local (see .gitconfig.local.template)"
    echo "  • Private settings: Add machine-specific aliases to ~/.config/zsh/private.sh"
    echo ""
